...
export class DatasourceConfigurationServices extends AbstractService<any> {
    constructor(private httpClient: HttpClient, private config: ConfigService) {
        super(httpClient, buildURL(config.getApiGateway() + `:${config.getApiPort()}/datasource/api/v2/configtype`));
    }
    ...
    findAllPaginateFilter(data: LazyLoadData | Partial<LazyLoadData>): Observable<ConfigType[]> {
        const params = map(data, (e, k) => (e !== undefined) ?
            ...
            .filter(f => f).join('&');
        const url = buildURL(this.config.getApiGateway()+`:${this.config.getApiPort()}/datasource/api/v2/configtype`);
