...
@Component({
    ...
    template: `
        <ng-container>
            <div *ngIf="root && item.visible !== false" class="layout-menuitem-root-text pb-0" (click)="itemClick($event)">{{ item.label }}</div>
            <a
                *ngIf="item.data.parent"
                [attr.href]="item.url"
            ...
            <span *ngIf="!item.data.parent"
                  routerLinkActive="active-link" [routerLinkActiveOptions]="{exact: true}"
                  class="flex justify-content-between align-items-center spanHover">
                 <a
                     class="flex flex-1"
                ...
                <i (click)="itemDeleteClick($event, item.data.id, item.data.type)"
                   class="flex pi pi-trash layout-submenu-toggler mr-2 text-red-700 cursor-pointer"
                   *ngIf="item.data.showDelete"></i>
            </span>
...
export class AppMenuitemComponent implements OnInit, OnDestroy {
    ...
    constructor(public layoutService: LayoutService,
                ...
                private datasourceServices: ConnectionsService,
    ...
    itemDeleteClick(event: Event, id: any, type: string) {
        ...
        this.confirmationService.confirm({
            ...
            accept: () => {
                ...
                if (type === 'BRAND') {
                    if(this.brandServices.selectedEntity$().id === id){
                        this.messageService.addError('You cannot delete the selected mark', 'Bad Request')
                    }else{
                        this.brandServices.delete(id);
                    }
                }
                if (type === 'DATASOURCE') {
                    this.datasourceServices.delete(id);
                }
            }
        });
